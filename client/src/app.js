...
// Tracks an in-flight token refresh to avoid multiple concurrent refresh calls.
...
async function apiFetch(path, options = {}, _isRetry = false) {
  ...
  // Silently refresh access token on expiry, then retry once.
  if (res.status === 401 && !_isRetry) {
    ...
    throw Object.assign(new Error(body.error || 'Unauthorized'), { data: body, status: 401 });
  }
...
async function doLogin(callback) {
  ...
  try {
    ...
    // Server sets httpOnly cookies — we only receive the user object
...
const App = {
  ...
  logout: async () => {
    ...
    // Cookies cleared by server; clear local state
    clearAuth();
    renderNav();
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Restore session from server (reads httpOnly cookie) while splash is shown
  await loadAuth();
