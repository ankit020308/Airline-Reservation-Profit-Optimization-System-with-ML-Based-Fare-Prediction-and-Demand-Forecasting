...
async function calculateFare({ flightId, cabinClass, paxCount = 1, userId, loyaltyTier = 'BLUE' }) {
  ...
  // Get inventory and flight from Postgres
  const { rows } = await db.readQuery(
    `SELECT fi.*, f.departure_time, f.status
     FROM flight_inventory fi
     JOIN flights f ON f.flight_id = fi.flight_id
     WHERE fi.flight_id = $1 AND fi.cabin_class = $2`,
    [flightId, cabinClass]
  );
  const row = rows[0];
  if (!row) return null;
  ...
  const inventory = row;
  const flight    = { departure_time: row.departure_time, status: row.status };
